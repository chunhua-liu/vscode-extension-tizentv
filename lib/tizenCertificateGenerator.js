...
function registerProfile(profileName, keyfileName, password, distributor) {
    let profile = profileEditor.findProfile(profileName);
    if (profile == undefined) {
        profile = profileEditor.newProfile(profileName);
    } else {
        profileEditor.removeProfile(profileName);
    }
    let encryptedPassword = p12ToPem.encryptPassword(password);

    profile = profileEditor.setProfileItem(profile, {
        distributor: distributor,
        key: keyfileName,
        password: encryptedPassword
    });
    profileEditor.writeProfile(profile);
}
...
function createCert(profileName, keyfileName, authorName, authorPassword, privilege){
    ...
    registerProfile(profileName, keyFilePath, authorPassword, '0');
    if (privilege == 'public') {
        registerProfile(profileName, distributorPublicSigner, distributorSignerPassword, '1');
    } else if (privilege == 'partner') {
        registerProfile(profileName, distributorPartnerSigner, distributorSignerPassword, '1');
    }
}
