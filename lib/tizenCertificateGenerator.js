...
function registerProfile(profileName, keyfileName, password, privilege) {
    profileEditor.createProfile(profileName, {
        key: keyfileName,
        password: p12ToPem.encryptPassword(password)
    }, {
        key: privilege == 'public' ? distributorPublicSigner : distributorPartnerSigner,
        password: p12ToPem.encryptPassword(distributorSignerPassword)
    }, undefined, true);

    //profileEditor.activateProfile(profileName);
}
